...
public class GanttChart {
    ...
    public static ArrayList<Pila> List = new ArrayList<Pila>(100); // list of gantt chart jobs' represnation
    ...
    public static void addJob(Job job , int time){
       ...
       Pila cell ;
       if(job == null) 
       {cell = Pila.createEmptyJobCell(ganttX, ganttY);} // represent empty job( white small cell)
       else 
       {
            ...
            cell = Pila.createGanttCell(ganttX, ganttY, job.jobNumber); 
       }
       ganttX += 11; // set next job location
       List.add(cell); // add cell to gantt chart list
       if( (time+1) % 10 == 0 ) 
       {
           List.add(Pila.createMark(ganttX -1 , ganttY+80));  // put small black mark every 10 times 
       }
    }
